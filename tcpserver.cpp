...
int Server::SendBroadcastMessage(int clientfd)
{
	...
	//判断接受到的信息是私聊还是群聊
	msg.fromID = clientfd;
	cout <<"private id"<< msg.type << endl;

	if (msg.type == 1) {
		msg.toID = msg.toID - '0';
		cout << "to id is " << msg.toID << endl;
	}
	...
	else
	{
		...
		//私聊
			...
			cout << clientfd << "chose to private" << endl;
			...
			for (it = clients_list.begin(); it != clients_list.end(); ++it) {		
				...
				eee[0] = b;
				strcat(msg.content, eee);
				strcat(msg.content, " is online\n");
			}
			cout <<"online client are"<< msg.content << endl;
			bzero(send_buf, BUF_SIZE);
			memcpy(send_buf, &msg, sizeof(msg));
			if (send(msg.fromID, send_buf, sizeof(send_buf), 0) < 0)
				return -1;		
